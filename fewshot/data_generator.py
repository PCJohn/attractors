"""
SOURCE: https://github.com/cbfinn/maml/blob/master/data_generator.py

This is code copied from the original MAML repository (https://github.com/cbfinn/maml)
...
class DataGenerator(object):
    ...
    def __init__(self,
        ...
        self.batch_size = 1 #meta batch size -- 1 as we're not doing maml here
        self.num_samples_per_class = num_samples_per_class
        self.num_classes = num_classes

        if 'omniglot' in self.datasource:
    ...
    def make_data_tensor(self, K, N, episode_len, test_set=False):
        train = (not test_set)
        if train:
        ...
        #return all_image_batches[0], all_label_batches[0]
        
        # Reshape to image shape
        im,label = all_image_batches[0], all_label_batches[0]
        S = im[:K*N]        # support set
        S_lab = label[:K*N] # support set labels
        B = im[K*N:]        # batch with novel samples
        B_lab = label[K*N:] # batch labels
        if self.datasource == 'omniglot':
