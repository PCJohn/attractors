...
class Conv():
    ...
    def insert(self,x):
        ...
        #self.memmat = (hebb - pre - post) / self.embed_dim
        #self.memmat = tf.expand_dims(tf.reduce_mean(self.memmat,axis=0),0)
    ...
    def recall(self,x):
        ...
        for s in range(self.inner_loop):
            ...
            #step = tf.matmul(self.memmat,x_s)
    ...
    def recall_batch(self,bx):
        out = []
        for i in range(self.bz*self.N): # TODO: Remove dependence on batch size
    ...
    def conv_features(self,conv_w,x):
        embed = x
        for cw in conv_w:
            #embed = tf.nn.conv2d(embed,cw[0],strides=[1,2,2,1],padding='SAME')
    ...
    def test(self, sess):
        ...
        for _ in range(self.num_meta_test_sets):
            ...
            meta_acc.append(acc)
        print(np.mean(meta_acc), np.std(meta_acc))


if __name__ == '__main__':
    K = 1
    N = 5
    episode_len = 2 # 2 novel sample per class -- the effective episode len is 2*N
    mem = True
    dataset = 'omniglot'
    output_dir = 'outputs'
    ...
    with tf.Session() as sess:
        data_generator = DataGenerator(K+episode_len,
                                       num_classes=N,
                                       datasource=dataset,
                                       test_set=(sys.argv[1]=='test'))
        S,S_lab,B,B_lab = data_generator.make_data_tensor(K,N,episode_len,test_set=(sys.argv[1]=='test'))
        
        print('>>>',S,S_lab,B,B_lab)
